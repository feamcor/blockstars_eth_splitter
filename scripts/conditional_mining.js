...
web3.eth.filter("latest", function(error, result) {
  ...
  if (txpool.status.pending > 0) {
    console.log(
      new Date() +
        "-- There were in fact pending transactions, so starting mining."
    );
    miner.start(1);
  }
});
...
if (txpool.status.pending) {
  console.log(
    new Date() + "-- Pending transactions on startup, so starting mining."
  );
  miner.start(1);
}

console.log(
  new Date() + "-- Started on-demand mining. Watching txpool for pending Txs.."
);
