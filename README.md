...
18. Continuing from the previous section, keep your terminal windows opened and Ganache running.
19. For running the DApp you will need MetaMask to be installed.
...
22. Run the DApp using the development server provided by React.

```bash
$ npm start
```

23. You should see similar output as below. Also, your default browser should open on the URL listed below.
...
24. After you import a few of the Ganache accounts to MetaMask, the latter should ask for authorization to connect the DApp to your account. Press `connect` to proceed.
   ![MetaMask connect dapp to account](screenshot_1.png)

25. The **Splitter** DApp is loaded and ready to be used. You certainly will see different screen than the example below which shows an account which current state all it to perform all operations: pause/unpause, split funds, split balance and withdraw. 

   ![Splitter DApp](screenshot_2.png)

26. **Congratulations!** Now you can use the **Splitter** DApp to split funds between two recipient accounts. You should have at least 3 accounts in MetaMask in order to play with the different roles that an account assume: pauser, funds donor and funds recipient. Enjoy!

## DAPP INSTALLATION (production)

27. The steps above enabled the development build of the DApp.
28. For production environments, you should use a production build where the app is packaged and optimized.
29. To generate the production build, run the command below.

```bash
$ npm run build
```

30. You should see similar output as below.
...
31. For running the production build, first you need to install a static server. Below is just one option, choose whatever is best to your environment or of your choice.

```bash
$ npm install serve --save
```

32. And then you can run the static server using your production build.

```bash
$ npx serve -s build
```

33. You should see similar output as below.
...
34. To interrupt the server, press `ctrl-c`
35. **Congratulations!** Now you can use the production build of the **Splitter** DApp. Enjoy!
